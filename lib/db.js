...
// CREATE TABLE IF NOT EXISTS only helps on a fresh database; a column added
// later needs adding to tables that already exist.
  ...
  if (!poolPromise) {
    poolPromise = (async () => {
      ...
      try {
        ...
        await ensureColumn(pool, 'articles', 'views', 'views INT UNSIGNED NOT NULL DEFAULT 0');
        await seed(pool);
      } catch (err) {
