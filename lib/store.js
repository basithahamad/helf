...
import { db, query, seedFile } from './db';
...
// document -- doing that overwrote the editor's work.
//
...
// document, so deleting a card or an event sticks.
...
export async function readSite() {
  ...
  const stored = rows.length
    ...
    : {};
  return withDefaults(seedFile('site.json') || {}, stored);
}
